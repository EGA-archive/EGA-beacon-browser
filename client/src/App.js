...
// import mockApi from "./components/mockApi.json";
...
function App() {
  ...
  const search = async (variant, genome, type = "original") => {
    ...
    try {
      if (USE_MOCK_API) {
        setMetaResults(mockApi.data?.response?.meta || []);
      } else {
        const metaresponse = await axios.get(`${config.API_URL}`, {
          headers: { "Content-Type": "application/json" },
        });
        setMetaResults(metaresponse.data.response);
      }
    ...
    try {
      ...
      const resultSets = USE_MOCK_API
        ? response.response?.resultSets || []
        : response.data.response.resultSets;

      console.log("USING MOCK:", USE_MOCK_API);
